...
class NysNewtonCG(Optimizer):
    ...
    def __init__(self, params, lr=1.0, rank=10, mu=1e-4, chunk_size=1,
                 cg_tol=1e-6, cg_max_iters=100, line_search_fn=None, verbose=False):
    ...
    if need_nncg:
        ...
        for epoch in range(nncg_max_iter):
            try:
                ...
                # 早停条件
                if grad_norm < 1e-6:
                    print(f"NNCG收敛：梯度范数 {grad_norm:.6e} < 1e-6")
                    break
                ...
                print("使用简单梯度下降作为备选...")
                # 简单梯度下降备选
                simple_optimizer = torch.optim.SGD(model.parameters(), lr=1e-4)
                for _ in range(100):
                    simple_optimizer.zero_grad()
                    loss, _, _, _, _, _, _ = compute_loss_strong(
                        model, domain_pts, boundary_left, boundary_right, boundary_top, boundary_bottom, boundary_hole
                    )
                    loss.backward()
        ...
        print(f"NNCG最终损失: {loss_history[-1]:.6f}")
