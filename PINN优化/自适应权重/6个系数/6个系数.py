...
# 细粒度自适应权重损失函数 - 方案1
def compute_loss_adaptive_fine(model, domain_points, boundary_left, boundary_right, boundary_top, boundary_bottom, boundary_hole,
                              sigma_pde, sigma_left, sigma_right, sigma_top, sigma_bottom, sigma_hole):
    """
    方案1：细粒度自适应权重损失函数（每个损失项独立的sigma参数）
    """
    # 1. 域内平衡方程残差: ∇·σ + f = 0
    ...
    # 细粒度自适应损失平衡 - 方案1：每个损失项独立的sigma
    adaptive_loss = (loss_pde / (2 * sigma_pde.pow(2)) + 
                    loss_bc_left / (2 * sigma_left.pow(2)) +
                    loss_bc_right / (2 * sigma_right.pow(2)) +
                    loss_bc_top / (2 * sigma_top.pow(2)) +
                    loss_bc_bottom / (2 * sigma_bottom.pow(2)) +
                    loss_bc_hole / (2 * sigma_hole.pow(2)) +
                    torch.log(sigma_pde) + torch.log(sigma_left) + 
                    torch.log(sigma_right) + torch.log(sigma_top) + 
                    torch.log(sigma_bottom) + torch.log(sigma_hole)) * 1e3
    
    return adaptive_loss, loss_pde, loss_bc_left, loss_bc_right, loss_bc_top, loss_bc_bottom, loss_bc_hole

# Adam优化训练函数 - 方案1 + 仅Adam
def train_adaptive_fine_adam_only(n_domain=6000, n_boundary=500, 
                                adam_epochs=10000, adam_lr=1e-3):
    """
    方案1：细粒度自适应权重 + 仅Adam优化器
    """
    ...
    # 初始化自适应权重参数 - 方案1：6个sigma
    sigma_pde = torch.tensor(2.0, dtype=torch.float32, device=DEVICE, requires_grad=True)
    sigma_left = torch.tensor(2.0, dtype=torch.float32, device=DEVICE, requires_grad=True)
    sigma_right = torch.tensor(2.0, dtype=torch.float32, device=DEVICE, requires_grad=True)
    sigma_top = torch.tensor(2.0, dtype=torch.float32, device=DEVICE, requires_grad=True)
    sigma_bottom = torch.tensor(2.0, dtype=torch.float32, device=DEVICE, requires_grad=True)
    sigma_hole = torch.tensor(2.0, dtype=torch.float32, device=DEVICE, requires_grad=True)
    
    # 优化器
    model_optimizer = torch.optim.Adam(model.parameters(), lr=adam_lr)
    sigma_optimizer = torch.optim.Adam([sigma_pde, sigma_left, sigma_right, 
                                       sigma_top, sigma_bottom, sigma_hole], lr=adam_lr)
    ...
    print(f"- 学习率: {adam_lr}")
    print(f"- 自适应参数数量: 6个sigma (PDE + 5个边界条件)")
    print(f"- 初始sigma值: 全部为2.0")
    print("=" * 60)
    ...
    for epoch in range(adam_epochs):
        ...
        # 计算损失
        loss, loss_pde, loss_bc_l, loss_bc_r, loss_bc_t, loss_bc_b, loss_bc_h = compute_loss_adaptive_fine(
        ...
        sigma_history['left'].append(sigma_left.item())
        sigma_history['right'].append(sigma_right.item())
        sigma_history['top'].append(sigma_top.item())
        sigma_history['bottom'].append(sigma_bottom.item())
        sigma_history['hole'].append(sigma_hole.item())
        
        weight_history['pde'].append((1/(2*sigma_pde.pow(2))).item())
        weight_history['left'].append((1/(2*sigma_left.pow(2))).item())
        weight_history['right'].append((1/(2*sigma_right.pow(2))).item())
        weight_history['top'].append((1/(2*sigma_top.pow(2))).item())
        weight_history['bottom'].append((1/(2*sigma_bottom.pow(2))).item())
        weight_history['hole'].append((1/(2*sigma_hole.pow(2))).item())
        
        # 记录各个损失项的值（用于分析）
        individual_loss_history['pde'].append(loss_pde.item())
        individual_loss_history['left'].append(loss_bc_l.item())
        individual_loss_history['right'].append(loss_bc_r.item())
        individual_loss_history['top'].append(loss_bc_t.item())
        individual_loss_history['bottom'].append(loss_bc_b.item())
        individual_loss_history['hole'].append(loss_bc_h.item())
        ...
        if epoch + 1 in check_points:
            ...
            print(f"  Left BC: {loss_bc_l.item():.6f}")
            print(f"  Right BC: {loss_bc_r.item():.6f}")
            print(f"  Top BC: {loss_bc_t.item():.6f}")
            print(f"  Bottom BC: {loss_bc_b.item():.6f}")
            print(f"  Hole BC: {loss_bc_h.item():.6f}")
            print(f"Sigma值:")
            print(f"  σ_PDE: {sigma_pde.item():.4f}, σ_Left: {sigma_left.item():.4f}")
            print(f"  σ_Right: {sigma_right.item():.4f}, σ_Top: {sigma_top.item():.4f}")
            print(f"  σ_Bottom: {sigma_bottom.item():.4f}, σ_Hole: {sigma_hole.item():.4f}")
            print(f"实际权重:")
            print(f"  w_PDE: {(1/(2*sigma_pde.pow(2))).item():.4f}, w_Left: {(1/(2*sigma_left.pow(2))).item():.4f}")
            print(f"  w_Right: {(1/(2*sigma_right.pow(2))).item():.4f}, w_Top: {(1/(2*sigma_top.pow(2))).item():.4f}")
            print(f"  w_Bottom: {(1/(2*sigma_bottom.pow(2))).item():.4f}, w_Hole: {(1/(2*sigma_hole.pow(2))).item():.4f}")
            
            # 计算权重分布（百分比）
            total_weight = sum([weight_history[key][-1] for key in weight_history.keys()])
            weight_percentages = {key: (weight_history[key][-1] / total_weight) * 100 for key in weight_history.keys()}
            print(f"权重分布(%):")
            print(f"  PDE: {weight_percentages['pde']:.1f}%, Left: {weight_percentages['left']:.1f}%")
            print(f"  Right: {weight_percentages['right']:.1f}%, Top: {weight_percentages['top']:.1f}%")
            print(f"  Bottom: {weight_percentages['bottom']:.1f}%, Hole: {weight_percentages['hole']:.1f}%")
        ...
        if epoch % 500 == 0:
            print(f"Epoch {epoch:5d}: Loss={loss_value:.6f}, "
                  f"PDE={loss_pde.item():.6f}, BC_avg={np.mean([loss_bc_l.item(), loss_bc_r.item(), loss_bc_t.item(), loss_bc_b.item(), loss_bc_h.item()]):.6f}")
    ...
    print(f"最终Sigma值:")
    for key in sigma_history.keys():
        print(f"  σ_{key}: {sigma_history[key][-1]:.4f}")
    print(f"最终权重值:")
    for key in weight_history.keys():
        print(f"  w_{key}: {weight_history[key][-1]:.4f}")
    
    return model, loss_history, sigma_history, weight_history, individual_loss_history

# 可视化细粒度自适应权重演化
def plot_fine_adaptive_weights_evolution(sigma_history, weight_history, individual_loss_history):
    """绘制方案1的细粒度自适应权重演化曲线"""
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    
    iterations = range(len(sigma_history['pde']))
    colors = ['black','blue', 'red', 'green', 'orange', '#767171']
    labels = ['PDE', 'Left', 'Right', 'Top', 'Bottom', 'Hole']
    

    
    # 所有sigma在一张图中
    for i, (key, label, color) in enumerate(zip(sigma_history.keys(), labels, colors)):
        axes[0].semilogy(iterations, sigma_history[key], color=color, linewidth=2, label=f'σ_{label}', alpha=0.7)
    axes[0].set_xlabel('Iteration')
    axes[0].set_ylabel('σ Values')#Sigma Value
    axes[0].set_title('  ')
    axes[0].grid(True, alpha=0.3)
    #axes[0].legend()
    
    # 实际权重演化
    for i, (key, label, color) in enumerate(zip(['pde', 'left', 'right','top', 'bottom', 'hole'], labels[:], colors[:])):
        axes[1].semilogy(iterations, weight_history[key], color=color, linewidth=2, label=f'w_{label}')
    axes[1].set_xlabel('Iteration')
    axes[1].set_ylabel('Actual Weight Values')
    axes[1].set_title('  ')
    ...
    # 左图：PDE, Left, Right损失
    for i, (key, label, color) in enumerate(zip(['pde', 'left', 'right','top', 'bottom', 'hole'], labels[:], colors[:])):
        axes[2].semilogy(iterations, individual_loss_history[key], color=color, linewidth=2, label=f'L_{label}')
    ...
    plt.tight_layout()
    plt.savefig('fine_adaptive_weights_evolution_adam_only.svg', format='svg', bbox_inches='tight')
    plt.show()
...
def visualize_results(model):
    ...
    plt.tight_layout()
    plt.savefig('final_displacement_fine_adaptive_adam.svg', format='svg', bbox_inches='tight')
    plt.show()
...
def plot_loss_history(loss_history):
    ...
    plt.ylabel('Loss')
    plt.title('Training Loss Evolution (Fine-Grained Adaptive Weights + Adam)')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('loss_history_fine_adaptive_adam.svg', format='svg', bbox_inches='tight')
    plt.show()

# 权重统计分析
def analyze_weight_evolution(weight_history):
    """分析权重演化统计信息"""
    print("\n=" * 60)
    print("权重演化统计分析")
    print("=" * 60)
    ...
    for key in weight_history.keys():
        ...
        print(f"{key.upper()} 权重:")
        print(f"  初始: {initial_weight:.6f}")
        print(f"  最终: {final_weight:.6f}")
        print(f"  最大: {max_weight:.6f}")
        print(f"  最小: {min_weight:.6f}")
        print(f"  变化倍数: {final_weight/initial_weight:.2f}x")
        print(f"  动态范围: {max_weight/min_weight:.2f}x")
        print()
...
if __name__ == "__main__":
    print("细粒度自适应权重PINNs - 带圆孔板拉伸问题")
    print("方案1：每个损失项独立的自适应权重 + Adam优化器")
    print("=" * 50)
    print(f"问题参数：")
    ...
    print(f"自适应参数: 6个sigma (PDE + 5个边界条件)")
    
    # 开始训练
    print(f"\n开始细粒度自适应权重训练...")
    model, loss_history, sigma_history, weight_history, loss_individual = train_adaptive_fine_adam_only(**params)
    
    # 绘制损失曲线
    plot_loss_history(loss_history)
    
    # 绘制细粒度自适应权重演化
    plot_fine_adaptive_weights_evolution(sigma_history, weight_history, loss_individual)
    
    # 权重演化统计分析
    analyze_weight_evolution(weight_history)
