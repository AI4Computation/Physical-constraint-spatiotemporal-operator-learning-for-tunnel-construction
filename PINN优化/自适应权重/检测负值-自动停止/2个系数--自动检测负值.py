...
def train_adaptive_adam_only(n_domain=6000, n_boundary=500, 
    ...
    # *** 新增：最优模型保存机制 ***
    best_loss = float('inf')  # 最小的正值adaptive loss
    best_model_state = None   # 最优模型状态
    best_sigma_pde = None     # 最优sigma_pde
    best_sigma_bc = None      # 最优sigma_bc
    best_epoch = 0            # 最优模型对应的epoch
    early_stopped = False     # 是否因负值而提前终止
    ...
    print("=" * 60)
    print("方案2：自适应权重 + Adam优化器训练（带最优模型保存）")
    print("=" * 60)
    print(f"训练参数：")
    ...
    for epoch in range(adam_epochs):
        ...
        # *** 检查是否出现负值，如果是则立即终止训练 ***
        if loss_value < 0:
            print(f"\n*** 警告：检测到负值损失 {loss_value:.6f} 在第 {epoch+1} 轮 ***")
            print(f"*** 立即终止训练并使用最优模型（第 {best_epoch+1} 轮，损失={best_loss:.6f}）***")
            early_stopped = True
            break
        
        # *** 更新最优模型（只考虑正值损失） ***
        if loss_value > 0 and loss_value < best_loss:
            best_loss = loss_value
            best_epoch = epoch
            # 深拷贝模型状态
            best_model_state = copy.deepcopy(model.state_dict())
            best_sigma_pde = sigma_pde.clone().detach()
            best_sigma_bc = sigma_bc.clone().detach()
            
            if epoch % 100 == 0 or epoch < 10:  # 前期和每100轮报告一次最优更新
                print(f"*** 更新最优模型：Epoch {epoch+1}, Loss={best_loss:.6f} ***")
        ...
        if epoch + 1 in check_points:
            ...
            print(f"最优损失: {best_loss:.6f} (第 {best_epoch+1} 轮)")
        ...
        if epoch % 500 == 0:
            print(f"Epoch {epoch:5d}: Loss={loss_value:.6f}, "
                  ...
                  f"Best={best_loss:.6f}@{best_epoch+1}")
    
    # *** 训练结束后加载最优模型 ***
    if best_model_state is not None:
        model.load_state_dict(best_model_state)
        sigma_pde.data = best_sigma_pde.data
        sigma_bc.data = best_sigma_bc.data
        print(f"\n*** 已加载最优模型：第 {best_epoch+1} 轮，损失={best_loss:.6f} ***")
    else:
        print(f"\n*** 警告：未找到有效的最优模型！***")
    ...
    if early_stopped:
        print(f"\n训练因负值损失提前终止！")
        print(f"实际训练轮数: {epoch+1}/{adam_epochs}")
    else:
        print(f"\n训练完成！")
        print(f"训练轮数: {adam_epochs}")
    
    print(f"总训练时间: {training_time:.2f} 秒")
    print(f"最终使用模型：第 {best_epoch+1} 轮")
    print(f"最优损失: {best_loss:.6f}")
    print(f"最优Sigma值 - PDE: {best_sigma_pde.item():.4f}, BC: {best_sigma_bc.item():.4f}")
    print(f"最优权重值 - PDE: {(1/(2*best_sigma_pde.pow(2))).item():.4f}, BC: {(1/(2*best_sigma_bc.pow(2))).item():.4f}")
    ...
    training_info = {
        ...
        'total_epochs': epoch + 1 if early_stopped else adam_epochs
    }
    
    return model, loss_history, sigma_history, weight_history, individual_bc_history, Pde_loss_history, training_info

# 可视化自适应权重演化
def plot_adaptive_weights_evolution(sigma_history, weight_history, individual_bc_history, Pde_loss_history, training_info):
    """绘制自适应权重演化曲线"""
    ...
    axes[0].axvline(x=best_epoch, color='green', linestyle='--', alpha=0.7, label=f'Best@{best_epoch+1}')
    ...
    axes[1].axvline(x=best_epoch, color='green', linestyle='--', alpha=0.7, label=f'Best@{best_epoch+1}')
    ...
    axes[2].axvline(x=best_epoch, color='green', linestyle='--', alpha=0.7, label=f'Best@{best_epoch+1}')
    axes[2].set_xlabel('Iteration')
    axes[2].set_ylabel('Loss')
    if early_stopped:
        axes[2].set_title(f'Loss Evolution (Early Stop @ {training_info["total_epochs"]})')
    else:
    ...
    plt.tight_layout()
    plt.savefig('adaptive_weights_evolution_adam_only_optimized.svg', format='svg', bbox_inches='tight')
    plt.show()
...
def visualize_results(model):
    ...
    axes[0].set_title('Final U Displacement (Optimal Model)')
    axes[0].set_aspect('equal')
    axes[0].set_xticks([-L, 0, L])
    axes[0].set_yticks([-L, 0, L])
    axes[0].tick_params(axis='both', pad=12)
    ...
    axes[1].set_title('Final V Displacement (Optimal Model)')
    axes[1].set_aspect('equal')
    axes[1].set_xticks([-L, 0, L])
    axes[1].set_yticks([-L, 0, L])
    axes[1].tick_params(axis='both', pad=12)
    ...
    plt.tight_layout()
    plt.savefig('final_displacement_adaptive_adam_optimized.svg', format='svg', bbox_inches='tight')
    plt.show()
...
# 绘制损失历史（改进版）
def plot_loss_history(loss_history, training_info):
    """绘制损失演化曲线"""
    ...
    plt.axvline(x=best_epoch, color='green', linestyle='--', alpha=0.7, linewidth=2, 
                label=f'Best Model @ Iter {best_epoch+1}')
    plt.scatter(best_epoch, loss_history[best_epoch], color='red', s=100, zorder=5, 
                label=f'Best Loss = {training_info["best_loss"]:.6f}')
    
    # 如果提前终止，标记终止点
    if training_info['early_stopped']:
        final_epoch = training_info['total_epochs'] - 1
        plt.axvline(x=final_epoch, color='red', linestyle=':', alpha=0.7, linewidth=2,
                   label=f'Early Stop @ Iter {final_epoch+1}')
    
    plt.xlabel('Iteration')
    plt.ylabel('Loss')
    if training_info['early_stopped']:
        plt.title(f'Training Loss Evolution (Early Stopped due to Negative Loss)')
    else:
        plt.title('Training Loss Evolution (Optimal Model Selection)')
    plt.grid(True, alpha=0.3)
    plt.legend()
    
    plt.tight_layout()
    plt.savefig('loss_history_adaptive_adam_optimized.svg', format='svg', bbox_inches='tight')
    plt.show()

# 主程序
if __name__ == "__main__":
    print("自适应权重PINNs - 带圆孔板拉伸问题")
    print("方案2：简化自适应权重 + Adam优化器 + 最优模型保存")
    print("=" * 50)
    print(f"问题参数：")
    ...
    # 开始训练
    print(f"\n开始自适应权重训练（带最优模型保存）...")
    model, loss_history, sigma_history, weight_history, bc_history, PDE_history, training_info = train_adaptive_adam_only(**params)
    
    # 绘制损失曲线
    plot_loss_history(loss_history, training_info)
    
    # 绘制自适应权重演化
    plot_adaptive_weights_evolution(sigma_history, weight_history, bc_history, PDE_history, training_info)
    
    # 可视化最终结果
    print("\n生成最终位移场（基于最优模型）...")
    visualize_results(model)
    
    # 计算并输出最终MSE、MAPE和R（基于最优模型）
    ref_x, ref_y, ref_u1, ref_u2 = load_reference_data()
    if ref_x is not None:
        ...
        print(f"\n最终误差指标结果（最优模型）：")
        print(f"最优模型来源：第 {training_info['best_epoch']+1} 轮训练")
        print(f"水平位移(U) - MSE: {mse_u:.8e}, MAPE: {mape_u:.4f}%, R: {r_u:.6f}")
        print(f"竖向位移(V) - MSE: {mse_v:.8e}, MAPE: {mape_v:.4f}%, R: {r_v:.6f}")
        
        if training_info['early_stopped']:
            print(f"\n注意：训练因负值损失在第 {training_info['total_epochs']} 轮提前终止")
            print(f"使用第 {training_info['best_epoch']+1} 轮的最优模型进行最终评估")
