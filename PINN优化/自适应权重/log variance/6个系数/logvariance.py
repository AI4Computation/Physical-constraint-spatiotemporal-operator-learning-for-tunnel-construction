...
# 方案1 - 细粒度Log Variance形式的自适应权重损失函数
def compute_loss_scheme1_log_variance(model, domain_points, boundary_left, boundary_right, 
                                    boundary_top, boundary_bottom, boundary_hole,
                                    s_pde, s_left, s_right, s_top, s_bottom, s_hole):
    """
    方案1：细粒度Log Variance形式的自适应权重损失函数
    
    参数：
    s_pde: log(ε_pde²) - PDE损失的log variance
    s_left: log(ε_left²) - 左边界损失的log variance
    s_right: log(ε_right²) - 右边界损失的log variance
    s_top: log(ε_top²) - 上边界损失的log variance
    s_bottom: log(ε_bottom²) - 下边界损失的log variance
    s_hole: log(ε_hole²) - 圆孔边界损失的log variance
    
    损失函数（按论文方程11的扩展形式）：
    L = 0.5*exp(-s_pde)*L_PDE + 0.5*exp(-s_left)*L_left + 0.5*exp(-s_right)*L_right +
        0.5*exp(-s_top)*L_top + 0.5*exp(-s_bottom)*L_bottom + 0.5*exp(-s_hole)*L_hole +
        s_pde + s_left + s_right + s_top + s_bottom + s_hole
    """
    ...
    # ===== 关键：细粒度Log Variance形式的自适应损失平衡（按论文方程11扩展） =====
    # L = 0.5*exp(-s_pde)*L_PDE + 0.5*exp(-s_left)*L_left + ... + s_pde + s_left + ...
    # 其中：s_x = log(ε_x²) 对于每个损失项
    adaptive_loss = (0.5 * torch.exp(-s_pde) * loss_pde + 
                    0.5 * torch.exp(-s_left) * loss_bc_left +
                    0.5 * torch.exp(-s_right) * loss_bc_right +
                    0.5 * torch.exp(-s_top) * loss_bc_top +
                    0.5 * torch.exp(-s_bottom) * loss_bc_bottom +
                    0.5 * torch.exp(-s_hole) * loss_bc_hole +
                    s_pde + s_left + s_right + s_top + s_bottom + s_hole)
    
    return adaptive_loss, loss_pde, loss_bc_left, loss_bc_right, loss_bc_top, loss_bc_bottom, loss_bc_hole

# Adam优化训练函数 - 方案1 + Log Variance + 单一优化器
def train_scheme1_log_variance(n_domain=6000, n_boundary=500, 
                             adam_epochs=10000, adam_lr=1e-3):
    """
    方案1：细粒度Log Variance + 单一Adam优化器
    """
    ...
    s_left = torch.tensor(1.386, dtype=torch.float32, device=DEVICE, requires_grad=True)     # log(ε_left²)
    s_right = torch.tensor(1.386, dtype=torch.float32, device=DEVICE, requires_grad=True)    # log(ε_right²)
    s_top = torch.tensor(1.386, dtype=torch.float32, device=DEVICE, requires_grad=True)      # log(ε_top²)
    s_bottom = torch.tensor(1.386, dtype=torch.float32, device=DEVICE, requires_grad=True)   # log(ε_bottom²)
    s_hole = torch.tensor(1.386, dtype=torch.float32, device=DEVICE, requires_grad=True)     # log(ε_hole²)
    
    # ===== 关键：单一优化器优化所有参数 =====
    all_params = list(model.parameters()) + [s_pde, s_left, s_right, s_top, s_bottom, s_hole]
    optimizer = torch.optim.Adam(all_params, lr=adam_lr)
    
    # 训练历史记录
    loss_history = []
    s_history = {
        'pde': [], 'left': [], 'right': [], 'top': [], 'bottom': [], 'hole': []
    }
    epsilon_history = {
        'pde': [], 'left': [], 'right': [], 'top': [], 'bottom': [], 'hole': []
    }
    ...
    print("=" * 60)
    print("方案1：细粒度Log Variance + 单一Adam优化器")
    print("=" * 60)
    print("关键特点：")
    print("- 使用log variance: s = log(ε²)")
    print("- 损失函数: L = 0.5*exp(-s_i)*L_i + Σs_i (6项)")
    print("- 单一Adam优化器优化网络参数和ε参数")
    print("- 无缩放因子（严格按论文）")
    print(f"- 参数数量: 6个log variance参数")
    print(f"- 初始log variance: 全部为{s_pde.item():.3f}")
    print(f"- 对应初始ε: 全部为{torch.sqrt(torch.exp(s_pde)).item():.3f}")
    print("=" * 60)
    ...
    for epoch in range(adam_epochs):
        ...
        # 计算损失
        loss, loss_pde, loss_bc_l, loss_bc_r, loss_bc_t, loss_bc_b, loss_bc_h = compute_loss_scheme1_log_variance(
        ...
        # 记录log variance值
        s_vals = [s_pde, s_left, s_right, s_top, s_bottom, s_hole]
        s_keys = ['pde', 'left', 'right', 'top', 'bottom', 'hole']
        
        for key, s_val in zip(s_keys, s_vals):
            s_history[key].append(s_val.item())
            
            # 转换为等效ε值 (ε = sqrt(exp(s)))
            epsilon_val = torch.sqrt(torch.exp(s_val)).item()
            epsilon_history[key].append(epsilon_val)
            
            # 计算实际权重 (0.5 * exp(-s))
            weight_val = (0.5 * torch.exp(-s_val)).item()
            weight_history[key].append(weight_val)
        
        # 记录各个损失项的值（用于分析）
        loss_vals = [loss_pde, loss_bc_l, loss_bc_r, loss_bc_t, loss_bc_b, loss_bc_h]
        for key, loss_val in zip(s_keys, loss_vals):
            individual_loss_history[key].append(loss_val.item())
        ...
        if epoch + 1 in check_points:
            ...
            print(f"各项损失值:")
            for key, loss_val in zip(s_keys, loss_vals):
                print(f"  {key.upper()}: {loss_val.item():.6f}")
            
            print(f"Log variance值:")
            for key, s_val in zip(s_keys, s_vals):
                print(f"  s_{key.upper()}: {s_val.item():.4f}", end="")
            print()
            
            print(f"等效ε值:")
            for key, s_val in zip(s_keys, s_vals):
                epsilon_val = torch.sqrt(torch.exp(s_val)).item()
                print(f"  ε_{key.upper()}: {epsilon_val:.4f}", end="")
            print()
            
            print(f"实际权重:")
            for key, s_val in zip(s_keys, s_vals):
                weight_val = (0.5 * torch.exp(-s_val)).item()
                print(f"  w_{key.upper()}: {weight_val:.4f}", end="")
            print()
            
            # 计算权重分布（百分比）
            total_weight = sum([(0.5 * torch.exp(-s_val)).item() for s_val in s_vals])
            print(f"权重分布(%):")
            for key, s_val in zip(s_keys, s_vals):
                weight_val = (0.5 * torch.exp(-s_val)).item()
                percentage = (weight_val / total_weight) * 100
                print(f"  {key.upper()}: {percentage:.1f}%", end="")
            print()
        ...
        # 定期输出训练进度
        if epoch % 500 == 0:
            avg_loss = np.mean([loss_val.item() for loss_val in loss_vals[1:]])  # 平均边界损失
            avg_epsilon = np.mean([torch.sqrt(torch.exp(s_val)).item() for s_val in s_vals])
            print(f"Epoch {epoch:5d}: Loss={loss_value:.6f}, "
                  f"PDE={loss_pde.item():.6f}, BC_avg={avg_loss:.6f}, ε_avg={avg_epsilon:.3f}")
    
    training_time = time.time() - start_time
    print(f"\n方案1 细粒度Log Variance训练完成！")
    print(f"总训练时间: {training_time:.2f} 秒")
    print(f"最终损失: {loss_history[-1]:.6f}")
    
    print(f"最终Log variance值:")
    for key, s_val in zip(s_keys, s_vals):
        print(f"  s_{key.upper()}: {s_val.item():.4f}")
    
    print(f"最终等效ε值:")
    for key, s_val in zip(s_keys, s_vals):
        epsilon_val = torch.sqrt(torch.exp(s_val)).item()
        print(f"  ε_{key.upper()}: {epsilon_val:.4f}")
    
    print(f"最终权重值:")
    for key, s_val in zip(s_keys, s_vals):
        weight_val = (0.5 * torch.exp(-s_val)).item()
        print(f"  w_{key.upper()}: {weight_val:.4f}")
    
    return model, loss_history, s_history, epsilon_history, weight_history, individual_loss_history

# 可视化细粒度Log Variance训练过程
def plot_fine_log_variance_evolution(s_history, epsilon_history, weight_history, individual_loss_history):
    """绘制方案1细粒度Log Variance形式的训练演化曲线"""
    fig, axes = plt.subplots(3, 3, figsize=(18, 15))
    
    iterations = range(len(s_history['pde']))
    colors = ['blue', 'red', 'green', 'orange', 'purple', 'brown']
    labels = ['PDE', 'Left', 'Right', 'Top', 'Bottom', 'Hole']
    
    # 第一行：Log variance演化
    # 左图：PDE, Left, Right
    for i, (key, label, color) in enumerate(zip(['pde', 'left', 'right'], labels[:3], colors[:3])):
        axes[0, 0].plot(iterations, s_history[key], color=color, linewidth=2, label=f's_{label}')
    axes[0, 0].set_xlabel('Iteration')
    axes[0, 0].set_ylabel('Log Variance (s)')
    axes[0, 0].set_title('Log Variance Evolution (PDE, Left, Right)')
    axes[0, 0].grid(True, alpha=0.3)
    axes[0, 0].legend()
    
    # 中图：Top, Bottom, Hole
    for i, (key, label, color) in enumerate(zip(['top', 'bottom', 'hole'], labels[3:], colors[3:])):
        axes[0, 1].plot(iterations, s_history[key], color=color, linewidth=2, label=f's_{label}')
    axes[0, 1].set_xlabel('Iteration')
    axes[0, 1].set_ylabel('Log Variance (s)')
    axes[0, 1].set_title('Log Variance Evolution (Top, Bottom, Hole)')
    axes[0, 1].grid(True, alpha=0.3)
    axes[0, 1].legend()
    
    # 右图：所有log variance在一张图中
    for i, (key, label, color) in enumerate(zip(s_history.keys(), labels, colors)):
        axes[0, 2].plot(iterations, s_history[key], color=color, linewidth=2, label=f's_{label}', alpha=0.7)
    axes[0, 2].set_xlabel('Iteration')
    axes[0, 2].set_ylabel('Log Variance (s)')
    axes[0, 2].set_title('All Log Variance Values')
    ...
    # 左图：PDE, Left, Right
    for i, (key, label, color) in enumerate(zip(['pde', 'left', 'right'], labels[:3], colors[:3])):
        axes[1, 0].plot(iterations, epsilon_history[key], color=color, linewidth=2, label=f'ε_{label}')
    axes[1, 0].set_xlabel('Iteration')
    axes[1, 0].set_ylabel('Epsilon Value')
    axes[1, 0].set_title('Epsilon Evolution (PDE, Left, Right)')
    axes[1, 0].grid(True, alpha=0.3)
    axes[1, 0].legend()
    
    # 中图：Top, Bottom, Hole
    for i, (key, label, color) in enumerate(zip(['top', 'bottom', 'hole'], labels[3:], colors[3:])):
        axes[1, 1].plot(iterations, epsilon_history[key], color=color, linewidth=2, label=f'ε_{label}')
    axes[1, 1].set_xlabel('Iteration')
    axes[1, 1].set_ylabel('Epsilon Value')
    axes[1, 1].set_title('Epsilon Evolution (Top, Bottom, Hole)')
    axes[1, 1].grid(True, alpha=0.3)
    axes[1, 1].legend()
    
    # 右图：权重分布饼图（最终状态）
    final_weights = [weight_history[key][-1] for key in weight_history.keys()]
    total_weight = sum(final_weights)
    percentages = [(w/total_weight)*100 for w in final_weights]
    
    wedges, texts, autotexts = axes[1, 2].pie(percentages, labels=labels, colors=colors, autopct='%1.1f%%', startangle=90)
    axes[1, 2].set_title('Final Weight Distribution')
    
    # 第三行：实际权重演化
    # 左图：PDE, Left, Right实际权重
    for i, (key, label, color) in enumerate(zip(['pde', 'left', 'right'], labels[:3], colors[:3])):
        axes[2, 0].semilogy(iterations, weight_history[key], color=color, linewidth=2, label=f'w_{label}')
    axes[2, 0].set_xlabel('Iteration')
    axes[2, 0].set_ylabel('Weight Value')
    axes[2, 0].set_title('Weight Evolution (PDE, Left, Right)')
    axes[2, 0].grid(True, alpha=0.3)
    axes[2, 0].legend()
    
    # 中图：Top, Bottom, Hole实际权重
    for i, (key, label, color) in enumerate(zip(['top', 'bottom', 'hole'], labels[3:], colors[3:])):
        axes[2, 1].semilogy(iterations, weight_history[key], color=color, linewidth=2, label=f'w_{label}')
    axes[2, 1].set_xlabel('Iteration')
    axes[2, 1].set_ylabel('Weight Value')
    axes[2, 1].set_title('Weight Evolution (Top, Bottom, Hole)')
    axes[2, 1].grid(True, alpha=0.3)
    axes[2, 1].legend()
    
    # 右图：加权损失演化（每项损失乘以其权重）
    for i, (key, label, color) in enumerate(zip(individual_loss_history.keys(), labels, colors)):
        weighted_loss = [individual_loss_history[key][j] * weight_history[key][j] 
                        for j in range(len(iterations))]
        axes[2, 2].semilogy(iterations, weighted_loss, color=color, linewidth=2, label=f'w×L_{label}', alpha=0.7)
    axes[2, 2].set_xlabel('Iteration')
    axes[2, 2].set_ylabel('Weighted Loss Value')
    axes[2, 2].set_title('Weighted Loss Evolution')
    axes[2, 2].grid(True, alpha=0.3)
    axes[2, 2].legend()
    
    plt.tight_layout()
    plt.savefig('scheme1_fine_log_variance_evolution.svg', format='svg', bbox_inches='tight')
    plt.show()
...
def visualize_results(model):
    ...
    axes[0].set_title('Final U Displacement (Fine Log-Var)')
    ...
    axes[1].set_title('Final V Displacement (Fine Log-Var)')
    ...
    plt.tight_layout()
    plt.savefig('final_displacement_scheme1_fine_log_variance.svg', format='svg', bbox_inches='tight')
    plt.show()

# 权重演化统计分析
def analyze_fine_weight_evolution(weight_history, epsilon_history):
    """分析细粒度权重演化统计信息"""
    print("\n=" * 60)
    print("细粒度权重演化统计分析（方案1）")
    print("=" * 60)
    
    for key in weight_history.keys():
        weights = weight_history[key]
        epsilons = epsilon_history[key]
        ...
        print(f"{key.upper()} 参数:")
        print(f"  权重: 初始={initial_weight:.6f}, 最终={final_weight:.6f}")
        print(f"  ε值: 初始={initial_epsilon:.4f}, 最终={final_epsilon:.4f}")
        print(f"  权重变化倍数: {final_weight/initial_weight:.2f}x")
        print(f"  权重动态范围: {max_weight/min_weight:.2f}x")
        print()
    ...
    print("最终权重分布:")
    for i, key in enumerate(weight_history.keys()):
        percentage = (final_weights[i] / total_weight) * 100
        print(f"  {key.upper()}: {percentage:.1f}%")
    
    print(f"\n权重集中度分析:")
    max_percentage = max([(w/total_weight)*100 for w in final_weights])
    min_percentage = min([(w/total_weight)*100 for w in final_weights])
    print(f"  最大权重占比: {max_percentage:.1f}%")
    print(f"  最小权重占比: {min_percentage:.1f}%")
    print(f"  权重不平衡度: {max_percentage/min_percentage:.2f}x")
...
def plot_loss_history(loss_history):
    ...
    plt.ylabel('Loss')
    plt.title('Training Loss Evolution (Scheme 1: Fine-Grained Log Variance)')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('loss_history_scheme1_fine_log_variance.svg', format='svg', bbox_inches='tight')
    plt.show()

# 主程序
if __name__ == "__main__":
    print("方案1：细粒度Log Variance + 单一优化器 - 带圆孔板拉伸问题")
    print("严格按照论文方程(11)扩展实现")
    print("=" * 50)
    print(f"问题参数：")
    ...
    print(f"自适应参数: 6个log variance (PDE + 5个边界条件)")
    
    # 开始训练
    print(f"\n开始方案1 细粒度Log Variance训练...")
    model, loss_history, s_history, epsilon_history, weight_history, loss_individual = train_scheme1_log_variance(**params)
    
    # 绘制损失曲线
    plot_loss_history(loss_history)
    
    # 绘制细粒度Log Variance演化
    plot_fine_log_variance_evolution(s_history, epsilon_history, weight_history, loss_individual)
    
    # 权重演化统计分析
    analyze_fine_weight_evolution(weight_history, epsilon_history)
