...
def calculate_weights(trunk_coords, branch_params, num_nearest=20, weight_factor=5.0):
    """
    璁＄畻姣忎釜鐐圭殑鏉冮噸锛岀妞渾涓績瓒婅繎鐨勭偣鏉冮噸瓒婂ぇ
    
    鍙傛暟:
    trunk_coords: 鏋佸潗鏍囩郴涓嬬殑鐐瑰潗鏍?(batch_size, num_points, 2)
    branch_params: 鍖呭惈姣忎釜鏍锋湰鍙傛暟鐨勫紶閲?(batch_size, 7)锛屽叾涓3鍜岀4涓弬鏁板垎鍒槸r鍜宲hi
    num_nearest: 闇€瑕佸姞鏉冪殑鏈€杩戠偣鏁伴噺
    weight_factor: 鏉冮噸鍊嶆暟
    """
    r = branch_params[:, 2]  # 绗?涓弬鏁版槸r
    phi = branch_params[:, 3]  # 绗?涓弬鏁版槸phi
    
    batch_size = trunk_coords.shape[0]
    weights = torch.ones_like(trunk_coords[:, :, 0])  # (batch_size, num_points)
    
    for i in range(batch_size):
        center_x = r[i] * torch.cos(phi[i])
        center_y = r[i] * torch.sin(phi[i])
        
        rho = trunk_coords[i, :, 0]  # 鏋佸緞
        theta = trunk_coords[i, :, 1]  # 鏋佽
        points_x = rho * torch.cos(theta)
        points_y = rho * torch.sin(theta)
        
        distances = torch.sqrt((points_x - center_x)**2 + (points_y - center_y)**2)
        
        valid_distances = distances[distances > 0]  # 鎺掗櫎濉厖鐨勭偣
        if len(valid_distances) > num_nearest:
            threshold = torch.sort(valid_distances)[0][num_nearest]
            weights[i][distances <= threshold] = weight_factor
    
    return weights

class WeightedMSELoss(nn.Module):
    def __init__(self, num_nearest=20, weight_factor=5.0):
        super(WeightedMSELoss, self).__init__()
        self.num_nearest = num_nearest
        self.weight_factor = weight_factor
    
    def forward(self, pred, target, trunk_coords, branch_input, mask):
        weights = calculate_weights(trunk_coords, branch_input, 
                                 self.num_nearest, self.weight_factor)
        ...
        total_valid_points = mask.sum()
        loss = weighted_error.sum() / total_valid_points
...
def train_model(model, train_loader, val_loader, optimizer, criterion, device, epochs=100):
    train_losses = []
    val_losses = []
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=300, eta_min=6.25e-05)
    ...
    def compute_validation_loss(loader):
        ...
        with torch.no_grad():
            for branch_input, trunk_input, target_output, lengths in loader:
                ...
                mask = mask < lengths.unsqueeze(1)
                loss = criterion(prediction, target_output, trunk_input, branch_input, mask)
...
if __name__ == "__main__":
    ...
    trunk_inputs = polar_coordinates(trunk_inputs)
    
    dataset = StressDataset(branch_inputs, trunk_inputs, outputs)
    ...
    criterion = WeightedMSELoss(num_nearest=20, weight_factor=3.0).to(device)
    
    train_losses, val_losses = train_model(model, train_loader, val_loader, optimizer, criterion, device, epochs=1000)
