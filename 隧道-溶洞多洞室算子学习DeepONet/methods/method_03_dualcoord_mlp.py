...
class DualDeepONet(nn.Module):
    def __init__(self, branch_input_dim, trunk_input_dim, hidden_dim, output_dim):
        ...
        self.fusion_net = nn.Sequential(
            nn.Linear(2, hidden_dim),  # 2 inputs (one from each DeepONet)
            nn.PReLU(),
    ...
    def forward(self, branch_input, trunk_input1, trunk_input2, lengths):
        """
        Input shapes:
            branch_input: (batch_size, 7)
            trunk_input1: (batch_size, max_points, 2)
            trunk_input2: (batch_size, max_points, 2)
            lengths: (batch_size,)
        """
        pred1 = self.deeponet1(branch_input, trunk_input1, lengths)
        pred2 = self.deeponet2(branch_input, trunk_input2, lengths)
        
        combined_preds = torch.stack([pred1, pred2], dim=-1)
        
        batch_size, max_points, _ = combined_preds.shape
        reshaped_preds = combined_preds.view(-1, 2)  # (batch_size * max_points, 2)
        
        output = self.fusion_net(reshaped_preds)
        
        output = output.view(batch_size, max_points)
    ...
    def __repr__(self):
        ...
        1. Input Processing:
           - Branch input (7 features) and Trunk inputs (2 features each)
           
        2. Parallel DeepONets:
           - DeepONet1: Processes original coordinate system
           - DeepONet2: Processes ellipse-centered coordinate system
           Both output (batch_size, max_points) tensors
           
        3. Fusion Network:
           - MLP combining predictions from both DeepONets
           - Progressive dimension reduction
           - Final single prediction per point
        """
